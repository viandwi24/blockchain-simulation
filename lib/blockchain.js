...
class Blockchain {
     ...
     */
    mine(mineAddress) {
        this.addTransaction((new Transaction(this.system.publicKey, mineAddress, this.blockMineReward)).sign(this.system.privateKey))
        const block = new Block(this.pendingTransactions, dayjs().toDate())
        block.previousHash = this.getLastBlock().hash
        block.mine(this.blockProofOfWorkDifficulty)
        this.chain.push(block)
        this.pendingTransactions = []
    }
